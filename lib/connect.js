const { default: simpleConnect, useMultiFileAuthState, patchMessageForMdIfRequired,jidDecode, DisconnectReason, generateWAMessageFromContent } = require("@adiwajshing/baileys")
...
exports.connect = async() => {
    ...
    samu330.prepareMessageFromContent = async (jid, content, options) => {
        console.log(content)
        let ptch = patchMessageForMdIfRequired(content)
        let gg = await generateWAMessageFromContent(jid, ptch, options)
        console.log(gg)
        console.log(JSON.stringify(gg.message, null, 2))
        samu330.relayMessage(jid, gg.message, {})
        return gg
    }
    samu330.prepareMessage = samu330.prepareMessageFromContent
    samu330.relayWAMessage = (jid, content, options = {}) => {
    /*if (jid.includes('@')) {
    samu330.relayMessage(jid, JSON.parse(JSON.stringify(content.message)), options)
} else if (!jid.includes("@")) {
    samu330.relayWAMessage(jid.key.remoteJid, jid.message, {})
    }*/
}
